...
pushd cf-deployment
	...
	bosh int iaas-support/bosh-lite/cloud-config.yml -o ../overide.yml -v network_name=${NETWORK_NAME}  -v network_range=${NETWORK_RANGE} -v network_gw=${NETWORK_GW} -v dns="[8.8.8.8]" >  cloud-config.yml
	...
	bosh -d cf deploy  cf-deployment.yml -o operations/bosh-lite.yml -o operations/use-compiled-releases.yml -v system_domain=$HOST_IP.xip.io --vars-store ~/cf-creds.yml  --no-redact  -n
popd
